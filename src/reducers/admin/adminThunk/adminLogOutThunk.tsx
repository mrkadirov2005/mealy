...
export const LogOutAdmin = createAsyncThunk<ADMIN[], PROPS>(
  'admin/fetchAdminLogOut',
  async ({ email, password }: PROPS) => {
    try {
      alert(email)
      alert(password)
      const response = await axios.post(`http://localhost:9000/auth`, {type:"admin/logout",email,password});
...
type FetchAdminAction = ReturnType<typeof LogOutAdmin.fulfilled>;
